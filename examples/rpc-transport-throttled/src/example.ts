...
/**
 * SETUP: CUSTOM THROTTLING TRANSPORT
 ...
 * Create a default RPC transport, wrap it in a throttled transport, then combine that with a
 * default Solana RPC API to create an RPC instance.
 */
const throttledRpc = createRpc({
    api: createSolanaRpcApi(DEFAULT_RPC_CONFIG),
    transport: getThrottledTransport(
        createDefaultRpcTransport({ url: mainnet('https://api.mainnet-beta.solana.com') }),
    ),
});

/**
 * STEP 1: MAKE 11 REQUESTS AT THE SAME TIME; CANCEL THE 8TH BEFORE IT STARTS
